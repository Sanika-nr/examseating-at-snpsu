...
// Import student rows from CSV upload
app.post('/api/import-students', async (req, res) => {
    ...
    try {
        const stmt = await db.prepare(`
            INSERT INTO students (srn, full_name, email, phone, dept_id, batch, academic_year, semester)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ...
        for (const student of students) {
            const { srn, full_name, email, phone, dept_id, batch, academic_year, semester } = student;
            if (!srn || !full_name) continue;
            await stmt.run([srn, full_name, email || '', phone || '', dept_id || 'CSE', batch || '2024-28', academic_year || '2024-25', semester || 1]);
            inserted++;
        }
        await stmt.finalize();
